...
function InvoiceDiscount(props) {
  const navigation = useNavigation();
  const [tabIndex, setTabIndex] = React.useState(0);
  const [discount, setDiscount] = React.useState('');
  const [discountCode, setDiscountCode] = React.useState('');
  const tabValues = ['GHS', '%'];
  const { setInvoiceDiscount } = useActionCreator();
  const { discountPayload, cart } = useSelector(state => state.invoice);
      ...
      <ScrollView style={{ backgroundColor: '#fff' }}>
        <View style={styles.main}>
          ...
          <View style={{ alignItems: 'center', marginVertical: 23 }}>
            <Text
              style={{ fontSize: 18, fontFamily: 'Lato-Bold', color: '#ddd' }}>
              Or
            </Text>
          </View>
          <View
        ...
        <PrimaryButton
          ...
          handlePress={() => {
            ...
            if (discountLength > 0 && discountCodeLength === 0) {
              ...
              if (tabValues[tabIndex] === '%') {
                setInvoiceDiscount({
                  discountType: tabValues[0],
                  quantity: Number(0.01 * Number(discount) * subTotal),
                });
              } else {
                setInvoiceDiscount({
                  discountType: tabValues[0],
