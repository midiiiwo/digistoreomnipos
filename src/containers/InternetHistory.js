...
import { useGetInternetHistory } from '../hooks/useInternetHistory';
...
const InternetHistory = () => {
  ...
  const { iStartDate, iEndDate } = useSelector(state => state.transactions);

  const { data, isFetching, isLoading, refetch } = useGetInternetHistory(
    user.merchant,
    user.login,
    user.user_merchant_group === 'Administrators',
    moment(iStartDate).format('DD-MM-YYYY'),
    moment(iEndDate).format('DD-MM-YYYY'),
  );
      ...
      <View style={{ backgroundColor: '#fff', paddingBottom: 12 }}>
        <View style={styles.topIcons}>
          <View style={styles.searchBox}>
            ...
            <Pressable
              onPress={() => {
                SheetManager.show('InternetFilter');
              }}>
