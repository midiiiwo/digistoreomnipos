...
const ExpensesHeader = ({ navigation }) => {
  const tabValues = ['Expenses', 'Categories'];
  const { activeTab } = useSelector(state => state.expenses);
  const { setActiveExpensesTab } = useActionCreator();
    ...
    <View style={[styles.headerMain, { paddingTop: top + 10 }]}>
      ...
      <View style={styles.segmentedControlWrapper}>
        <SegmentedControl
          ...
          onChange={event => {
            setActiveExpensesTab(event.nativeEvent.selectedSegmentIndex);
          }}
