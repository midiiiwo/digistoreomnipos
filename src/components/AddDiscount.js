...
const initialState = {
    ...
    selectedDiscountValue: 'AUTOMATIC',
    requiredValue: '',
    ...
    return (
        <View style={{ flex: 1, backgroundColor: "#fff" }}>
            ...
            <ScrollView style={styles.main}>
                <View>
                    {/* <Text style={{ color: 'gray' }}>Setup your discounts; assign discounts to your products, outlet(s) or shop(s) customers can buy from all in ONE ACCOUNT</Text> */}
                    <View>
                        {state.selectedDiscountValue === "lol" && discount_id == undefined && (
                        ...
                        <Input
                            ...
                            placeholder={discount_id ? discountDetails?.data?.data?.discount_code : 'Enter Discount Name eg:(Black Friday promo)'}
                            editable={discount_id ? false : true}
                        />
                        <Text style={{ color: "gray" }}>Customers will see this in cart and at checkout.</Text>
                    </View>
                    <Divider />
                            ...
                            {(state.selectedAppliesTo === 'SELECTPRODUCTS' ||
                                (discount_id && discountDetails?.data?.data?.discount_apply === 'SELECTPRODUCTS')) && (
                                    <View style={{ backgroundColor: "rgba(25, 66, 216, 0.87)", alignItems: 'center', borderRadius: 50, padding: 5, width: 140 }}>
                                        <Pressable onPress={() => navigation.navigate('Productlist', { discount_id })}>
                            ...
                            {(state.selectedAppliesTo === 'DELIVERYROUTES' ||
                                (discount_id && discountDetails?.data?.data?.discount_apply === 'DELIVERYROUTES')) && (
                                    <View style={{ backgroundColor: "rgba(25, 66, 216, 0.87)", alignItems: 'center', borderRadius: 50, padding: 5 }}>
                                        <Pressable onPress={() => navigation.navigate('Deliverylist', { discount_id })}>
            ...
            <View style={styles.btnWrapper}>
                ...
                >
                    {discount_id
                        ? (isLoadingEditDiscount ? "Processing" : "Edit Discount")
                        : (isLoadingDiscount ? "Processing" : "Create Discount")}
                </PrimaryButton>
            </View>
        </View>
    );
